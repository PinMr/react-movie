/**
 * Created by billbear on 2017/5/11.
 */
//公共的
export const COMMON_CITY = 'COMMON_CITY' //城市
export const COMMON_SIDEMENU_STATUS = 'COMMON_SIDEMENU_STATUS' //侧边栏状态
export const COMMON_TITLE = 'COMMON_TITLE' //标题
export const COMMON_LOADING_STATUS = 'COMMON_LOADING_STATUS' //加载状态
export const COMMON_ERROR_STATUS = 'COMMON_ERROR_STATUS' //错误状态

// 首页
export const HOME_BANNER = 'HOME_BANNER' //首页banner图
export const HOME_NOW_PLAYING = 'HOME_NOW_PLAYING' //首页正在上映电影数据
export const HOME_COMING_SOON = 'HOME_COMING_SOON' //首页即将上映电影数据